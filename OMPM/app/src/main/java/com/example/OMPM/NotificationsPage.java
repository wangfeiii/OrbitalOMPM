        ...
        mDatabase.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (DataSnapshot ds: dataSnapshot.child("users").child(user.getUid()).child("owedTo").getChildren()) {
                    ...
                    String creditor = String.valueOf(sc.child("name").getValue());
                    for (DataSnapshot debtor : sc.child("debtors").getChildren()) {
                        ...
                        Debt debt = new Debt(key, String.valueOf(sc.child("amount").getValue()), String.valueOf(sc.child("date").getValue()), map.get("phone"),creditor,Boolean.parseBoolean(paid));
                        if (paid.equals(false)) {
                            mDebtList.add(debt);
                            Log.d(TAG, mDebtList.get(0).getAmount());
                        }
                    }
                }
